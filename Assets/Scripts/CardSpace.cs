...
public class CardSpace : MonoBehaviour, IDropHandler {
	...
	public bool AttemptToPlaceCard(CardDisplay cardDisplay, System.Action OnAnimEnd = null){
		bool WasPlaced = false;

		if(cardDisplay == null || cardDisplay.HasBeenPlayed){
			return false;
		}
			...
			if(CanPlaceCard(cardDisplay.card) ){
				...
				if(GM.CanBuyCard(cardDisplay)){
					if(OnAnimEnd == null){ PlaceCard(cardDisplay); } else
										 { PlaceCard(cardDisplay, OnAnimEnd); }
					GM.CurrentAction.Clean();
					GM.CurrentAction.movementType = TurnMovementType.CardPurchase;
					GM.CurrentAction.CardInAction = cardDisplay;
					GM.CurrentAction.PurchasePrice = cardDisplay.card.Cost;
					GM.CurrentAction.HandIndexOrigin = cardDisplay.HandIndex;
					cardDisplay.SetPurchaseAction(GM.RegisterCurrentAction());
					Occupied = true;
					WasPlaced = true;
				} else {
	...
	private void PlaceCard(CardDisplay card, System.Action OnAnimEnd = null){
		...
		if(Owner.useAI){
			// Debug.Log(OnAnimEnd);
			if(OnAnimEnd != null){
				card.rectTransform.LeanMove(transform.position, 0.25f).setEaseOutQuart().setOnComplete(delegate(){OnAnimEnd();});
			} else {
				//Debug.Log("No anim end for " +card.card.Name);
				card.rectTransform.LeanMove(transform.position, 0.25f).setEaseOutQuart();
			}
		} else {
